...
const entries = [
  ...source.matchAll(/^\s+'([^']+)':\s*\n\s+'([^']+)'/gm),
  ...source.matchAll(/^\s+([a-z][a-z0-9-]*):\s*\n\s+'([^']+)'/gm),
].map(([, id, url]) => ({ id, url }))
...
const BAD_BODY = /page not found|not found|\bsorry\b|cannot be found|error 404|>404</i
...
async function check({ id, url }) {
  try {
    ...
    if (BAD_BODY.test(body) || BAD_BODY.test(title)) {
